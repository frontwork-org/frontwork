"use strict";
(() => {
  // frontwork-std/test/dist/main.front.bundle.js
  ...
  var I18n = class {
    ...
    set_locale(locale) {
      ...
      if (locale_found === void 0)
        throw new Error("I18nLocale " + locale + " does not exist");
  ...
  var I18nLocale = class {
    ...
    get_translation(key) {
      ...
      if (translation === void 0)
        throw new Error('I18nLocale.get_translation("' + key + '"): can not get translation, because the specific key does not exist.');
    ...
    routes_resolver(request) {
      ...
      for (let i = 0; i < this.domain_routes.length; i++) {
        ...
        if (domain_routes1.domain.test(request.host)) {
          for (let i2 = 0; i2 < domain_routes1.routes.length; i2++) {
            const route = domain_routes1.routes[i2];
            ...
            if (request.path_dirs.length === route_path_dirs.length) {
              ...
              for (let i3 = 0; i3 < route_path_dirs.length; i3++) {
                ...
                if (route_path_dir !== "*" && route_path_dir !== request.path_dirs[i3]) {
                  found = false;
                  break;
                }
              }
  ...
  var FrontworkMiddleware = class {
    ...
    log(request, extra) {
      ...
      if (request.query_string !== "")
        path_with_query_string += "?" + request.query_string;
      console.log(request.method + " " + path_with_query_string + " " + extra);
      if (request.POST.items.length > 0)
        console.log(" Scope POST: ", request.POST.items);
    }
  };
  var FrontworkFront = class extends Frontwork {
    request_url;
    build_on_page_load;
    constructor(init3, front_init) {
      super(init3);
      this.request_url = location.toString();
      if (typeof front_init.build_on_page_load === "boolean")
        this.build_on_page_load = front_init.build_on_page_load;
      else
    ...
    page_change(savestate, do_building) {
      ...
      if (result.response !== null) {
        ...
        if (typeof resolved_content.document_html !== "undefined") {
          ...
          if (result.dom_ready !== null)
            result.dom_ready(context, this);
          return {
            url: this.request_url,
            is_redirect: false
          };
  ...
  var __default1 = JSON.parse(`[
    { "key": "title1", "translation": "Frontwork Test Seite" }
    ,{ "key": "text1", "translation": "Dies ist eine deutsche Test Seite f\xFCr das Frontwork framework." }
